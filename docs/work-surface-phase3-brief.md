...
Updated 2026-09-18. Check an item only after it exists in the working implementation; future amendments should be added here when completed.
...
17. **Photography block tile shows pictures.** (2026-09-18 PM.) On the unfiltered map the Photography-tagged series collapse into one tile, and that tile is a 3×3 mosaic, never an empty box: hero cell (2×2, top-left) plus five cells, each the `thumbnail` of one series, each carrying the series name in JetBrains Mono 10px `#B4B6C2` on a 55% dark chip at its top-left. More than six series → the last cell shows `+N`. Caption: `Photography` / `Series · N series · portrait, editorial, film` (N computed). Click → `/work/photography`. `--photography` or `--creative-direction` releases the series into individual tiles on the main map. Saints and Fractured are not Photography-tagged and stay as their own tiles at all times.
