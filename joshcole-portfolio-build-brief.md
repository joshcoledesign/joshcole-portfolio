...
6. **Doors (sticky bottom nav bar — this is the site-wide navigation):**
   ...
   - Below it, 3 columns: **The Thread** (sub: "THE PLOT"), **The Volumes** (sub: "THE CHAPTERS"), **About** (sub: "THE AUTHOR"). Sublabels carry the book metaphor, parallel "THE ___" rhythm, mono ~12px muted. Square corners, near-black surface — must read as part of the dossier/terminal world, NOT generic app chrome.
...
### Navigation note
The homepage hero has **no persistent top nav** — the prompt line owns the top bar. **Navigation site-wide is the sticky bottom doors bar** (see hero item 6): The Thread / The Volumes / About, with the `>` prefix + hover-cursor link affordance and the scroll-collapse behavior. This is the single nav system on every page — homepage and inner pages alike. The earlier plan for a separate inner-page persistent *top* nav is retired; build the sticky bottom bar as one shared component used everywhere, so there is never both a top nav and the bottom bar on the same page.
